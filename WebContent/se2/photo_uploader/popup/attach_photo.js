		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = false; //버그가 있어서 false로 수정함
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/Megabox/file/fileUpload',	//샘플 URL입니다.
 	        sCallback : '/Megabox/se2/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
