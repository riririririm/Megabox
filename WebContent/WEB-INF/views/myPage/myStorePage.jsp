		...
		<div class="myPage">
			<div class="myPage_nav">
				<ul style="padding-left: 0px;">
					...
					<li><a
						href="${pageContext.request.contextPath}/member/myPersonalPage">
			...
			<div id="store_table">
				...
				<form>
					<table class="store_content table">
						...
						<c:if test="${buyList.size() eq 0}">
							<tr>
								<td colspan="8"
									style="padding-top: 130px; text-align: center; height: 300px;">등록된
									구매내역이 없습니다.</td>
							</tr>
						</c:if>
						<c:forEach items="${buyList}" var="buy" varStatus="i">
							<!-- list안에 있는 것을 하나 꺼내서 DTO에 집어 넣는다 -->
							<c:if test="${buy.state eq 0}">
								<tr>
									...
									<td>${buy.buy_count}</td>
									<td>${buy.expiry_date}까지
										<button type="button" title="${buy.history_num}"
											style="margin-left: 10px" id="btn_modal" data-toggle="modal"
											class="btn_buy" data-target="#myModal">취소</button>
									</td>
								</tr>
							</c:if>
						</c:forEach>
					...
					<div>
						<ul class="pager">
							<c:if test="${pager.curBlock gt 1}">
								<li class="previous"><a
									href="./myStorePage?curPage=${pager.startNum-1}">Previous</a></li>
							</c:if>
							<li>
								<ul class="pagination">
							...
							<c:if test="${pager.curBlock lt pager.totalBlock}">
								<li class="next"><a
									href="./myStorePage?curPage=${pager.lastNum+1}">Next</a></li>
							</c:if>
						</ul>
					</div>
				</c:if>
				<div>
					<ul class="">
						<li>스토어 상품은 부분 환불이 불가합니다.<br>(ex. 3개의 쿠폰을 한 번에 구매하신 경우,
							3개 모두 취소만 가능하며, 그 중 사용하신 쿠폰이 있는 경우 환불이 불가합니다.)
						</li>
					...
					<div>
						<ul class="pager">
							<c:if test="${cancelPager.curBlock gt 1}">
								<li class="previous"><a
									href="./myStorePage?cancelPage=${cancelPager.startNum-1}">Previous</a></li>
							</c:if>
							<li>
								<ul class="pagination">
									<c:forEach begin="${cancelPager.startNum}"
										end="${cancelPager.lastNum}" step="1" var="i">
							...
							<c:if test="${cancelPager.curBlock lt cancelPager.totalBlock}">
								<li class="next"><a
									href="./myStorePage?cancelPage=${cancelPager.lastNum+1}">Next</a></li>
							</c:if>
						</ul>
					</div>
				</c:if>
			...
			<div class="modal-content" style="width: 500px">
				...
				<div class="modal-body">
					...
					<div style="margin-top: 50px">
						<button type="button" id="cancelStore" class="btn btn-default"
